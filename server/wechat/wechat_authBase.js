...
export const InjectSimulateUserSession = async(req, res, next) => {
    ...
    //获取微信用户身份，绑定土猴身份信息
    const userInfo = await User.findAndCreateUserFromWeixinOpnId(global.weixin_user);
...
router.all('*', async function(req, res, next) {
    ...
    if(!code || needGoAuthorizeURL) {
        ...
        const url2 = oauthApi.getAuthorizeURL(encodeURI(url), stateNew, 'snsapi_base');
        ...
        try {
            ...
            //获取微信用户身份，绑定土猴身份信息
            req.session.user = {};
            req.session.user.wx_openID = AccessTokenInfo.openid;
