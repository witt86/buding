...
const handleMessage_text = wechat( _config.wxconfig, async function (req,res,next){
    ...
    try {
        if (MsgType=='text') {
            // const phone=text.substring(0,text.length-4);
            // const orderId=text.slice(-4);
            // if (text == '我要开店') {
            //     let Articles = require("./../../config/weixin_invite_guide2join.json");
            //     Articles=  RenderJSONTemplate(Articles,{"_config.sitehost":_config.sitehost  });
            //     res.reply(Articles);
            // } else if (text == '我是供应商') {
            //     res.reply("请点击这里完成供应商身份验证: " + _config.sitehost + "/guest/supplierregister");
            // } else if (text == '工具箱') {
            //     res.reply("仅限管理员访问: " + _config.sitehost + "/admin");
            // } else if (text=='企业注册'){
            //     res.reply("点击这里注册企业："+_config.sitehost + "/guest/leadorgregister/0");
            // } else if (text=="对账单") {
            //     const supplierAccount = await DataModel.SupplierAccount.findOne({where: {wx_openID: req.weixin.FromUserName}});
            //     if (supplierAccount) {
            //         res.reply(`请点击这里查看您的账单:${ _config.sitehost }/supplier/reportList/${ supplierAccount.uid }`);
            //     } else {
            //         res.reply(`您还未绑定供应商账号,点击这里绑定:${ _config.sitehost }/guest/supplierregister`);
            //     }
            // } else if (/^1[3|4|5|7|8]\d{9}$/.test(phone) && orderId.length==4){
            //     const order=await DataModel.Buyment.findAll({ where:{ orderID:{ '$like': `%${orderId}%` } },attributes:['orderID'] });
            //     let orderid=0
            //     if (order && order.length>0){
            //         orderid=order[0].orderID?order[0].orderID:0;
            //     }
            //     res.reply(`点击这里查看订单物流信息:${ _config.sitehost }/saleshop/user/ordershipping/${orderid}/0?mobile=${phone}&suffix=${orderId}`)
            // }
            // else {
            //     res.reply();
            // }
            res.reply('http://haoli.yichihui.com/shop/shopHome');
        } else {
            next();
        }
    }catch (e){
...
const handleMessage_event = wechat( _config.wxconfig, function (req,res,next){
    ...
    //         if (Event == "subscribe") {
    //             delayRun(async() => {
    //                 await BusinessEventDynlib.handleUserEvent({
    //                     wxopenid: req.weixin.FromUserName,
    //                     eventName: types.SERVER_EVENT_SUBSCRIBE
    //                 });
    //             }, 1000, (err) => {
    //                 console.error(`BusinessEventDynlib.handleUserEvent...err:${err}`);
    //             });
    //         }
