...
export const getPageWXShareInfo = ()=> {
    if (typeof window == "object") {
        ...
        return {
            title: "爱游购",
            content: "让你的旅行购物更轻松!",
            link: "http://itravelbuy.twohou.com",
            imgurl: "http://itravelbuy.twohou.com/images/icon_108_108.png"
        };
    }
}
...
export const wxready=()=>{
    ...
    window.wx.onMenuShareTimeline({
        ...
        success: ()=> {
            // 用户确认分享后执行的回调函数
            if (window.onWxShare)
                window.onWxShare(true);
        },
        cancel: ()=> {
            // 用户取消分享后执行的回调函数
            if (window.onWxShare)
                window.onWxShare(false);
        }
    });
    /*分享给朋友*/
...
export const  updateFile2Qiniu= async ( prefix, file_elem,  qiniutoken, filename)=>{
    ...
    const result=await http.post("http://upload.qiniu.com",data);
    if (result){
